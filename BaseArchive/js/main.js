...
function n(d) {
  ...
  for (var a = 0, k = d.length; a < k; a++) {
    ...
    var m = "";
    c.innerHTML += "<td><a style='color: #e6e6e6;' target='_blank' href='/BaseArchive/" + d[a][0] + "'>" + d[a][0] + "</a><br><span class=\"badge badge-info\">" + d[a][5] + "</span></td>";
    ...
    success: function (c) {
      ...
      a.onmessage = function (c) {
        function download(filename, text) {
          ...
          download("BaseArchive-data.json", c.data);
        g = JSON.parse(c.data);
...
var F = $.ajax({
  ...
  success: function () {
    ...
    var a = localStorage.getItem("LLBoad");
    var b = localStorage.getItem("LLBoadT");
    if (a == null || b == null || a != d) {
      localStorage.setItem("LLBoad", d);
